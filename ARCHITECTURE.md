...
- **Dashboard:** `App\Livewire\Dashboard` prepares role-aware quick actions, upcoming events placeholders, RSVP and activity slots so future event/comment modules can populate them without layout churn. Group listings feed into this ecosystem through the shared DaisyUI layout.

## Testing Notes
- Role and auth coverage spans `tests/Feature/RoleSystemTest.php`, the Fortify suites, and expanded registration/profile tests (avatar storage, metadata updates). Session middleware is exercised implicitly through the 2FA/password confirmation flows, while group CRUD and membership logic is covered in `tests/Feature/Groups`.
